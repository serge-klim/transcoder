...
struct decoder<T, Options, detail::trivially_encodable<T>> {
...
struct decoder<std::array<T, N>, Options, detail::trivially_encodable<T>> {
...
template<typename ...T, typename Options>
struct decoder<std::tuple<T...>, Options, typename std::conjunction<detail::trivially_encodable<T>...>::type> {
...
private:
	...
	std::tuple<T...> decode(boost::mp11::mp_list<Offset...>, boost::mp11::mp_list<Sizeof...>, byte_t const*& begin, [[maybe_unused]] byte_t const* end) {
		...
		auto tmp = begin;
		begin += size;
		return { decode_element<T, Sizeof>(tmp + Offset::value) ..., };
	}
...
template<typename T, typename Options>
struct decoder<T, Options, typename boost::describe::has_describe_members<T>::type> {
	T operator()(byte_t const*& begin,byte_t const* end) noexcept {
	  using struct_sizeof = decltype(members_sizeof(std::declval<boost::describe::describe_members<T, boost::describe::mod_public>>(), 0));
      if constexpr (std::is_same_v<std::size_t, struct_sizeof>) {
...
private:
	template <template <class...> class L, typename... Descriptor>
	auto members_sizeof(L<Descriptor...>, int) -> decltype((encoded_member_sizeof<Descriptor::pointer, Options>::value + ...));
	template <template <class...> class L, typename... Descriptor>
	void members_sizeof(L<Descriptor...>, ...);
	...
	T decode(L<Descriptor...>, boost::mp11::mp_list<Offset...>, boost::mp11::mp_list<Sizeof...>, byte_t const*& begin, [[maybe_unused]] byte_t const* end) {
		...
		auto tmp = begin;
		begin += size;
		return { decode_member<Descriptor, Sizeof>(tmp + Offset::value) ...,};
	}
