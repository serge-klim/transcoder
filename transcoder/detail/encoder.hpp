...
struct encoder<T, Options, detail::trivially_encodable<T>> {
...
struct encoder<std::array<T, N>, Options, detail::trivially_encodable<T>> {
...
template <typename T, std::size_t N, typename Options>
struct encoder<T[N], Options, detail::trivially_encodable < T >>
{
...
template<typename ...T, typename Options>
struct encoder<std::tuple<T...>, Options, typename std::conjunction<detail::trivially_encodable<T>...>::type> {
...
template<typename T, typename Options>
struct encoder<T, Options, typename boost::describe::has_describe_members<T>::type> {
	byte_t* operator()(T const& value, byte_t* begin, byte_t* end) noexcept {
		[[maybe_unused]] static constexpr auto encoded_size = encoded_sizeof<T, Options>::value;
		assert(std::distance(begin, end) >= encoded_size);
